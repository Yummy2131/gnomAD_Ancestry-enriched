    ...
    for filename in os.listdir(input_folder):
        if filename.endswith('.xls') or filename.endswith('.xlsx'):
            ...
            try:
                ...
                Group_A_Enriched = df[(df[group_column] == Group_A) &
                                      (df[A_frequency_column] > 0.00005) &
                                      (df[A_frequency_column] < 0.01) &
                                      (df[A_allele_count] > 2) & ((df[A_to_B_frequency_ratio_column] >= 10) |
                                                                  (df[A_to_B_frequency_ratio_column].isna()))]

                Group_B_Enriched = df[(df[group_column] == Group_B) &
                                      (df[B_frequency_column] > 0.00005) &
                                      (df[B_frequency_column] < 0.01) &
                                      (df[B_allele_count] > 2) & ((df[B_to_A_frequency_ratio_column] >= 10) | (
                    df[B_to_A_frequency_ratio_column].isna()))]
                ...
                Group_A_Conflicting = filtered_A[
                    (filtered_A[clinVar_annotation] == 'Conflicting classifications of pathogenicity')]
                ...
                Group_B_Conflicting = filtered_B[
                    (filtered_B[clinVar_annotation] == 'Conflicting classifications of pathogenicity')]
                ...
                Group_A_VUS_Enriched = Group_A_Enriched[
                    (Group_A_Enriched[clinVar_annotation] == 'Uncertain significance')]
                ...
                Group_A_pLOF_Enriched = Group_A_Enriched[(Group_A_Enriched['VEP Annotation'] == 'frameshift_variant') |
                                                         ...
                                                         (Group_A_Enriched[
                                                              'VEP Annotation'] == 'splice_acceptor_region') |
                ...
                Group_B_VUS_Enriched = Group_B_Enriched[
                    (Group_B_Enriched[clinVar_annotation] == 'Uncertain significance')]

                Group_B_Conflicting_Enriched = Group_B_Enriched[
                    (Group_B_Enriched[clinVar_annotation] == 'Conflicting classifications of pathogenicity')]
                ...
                print(
                    f"{filename}: {Group_A_row_count} rows {Group_A}, {Group_B_row_count} rows {Group_B} match the conditions")
...
input_folder = f'C:\\Users\\boxx_\\Desktop\\gnomAD\\{file_path}'    # Folder containing the input Excel files
output_folder = f'C:\\Users\\boxx_\\Desktop\\gnomAD\\{file_path}'  # Folder where the filtered Excel files will be saved
output_file = f'C:\\Users\\boxx_\\Desktop\\gnomAD\\{file_path}\\NFE_AMR_{file_path}.xlsx'
group_column = 'GroupMax FAF group' #'Genome GroupMax FAF group' #
Group_A = 'nfe' #nfe or amr or afr
Group_B = 'amr' #nfe or amr or afr
A_frequency_column = 'Frequency European non-Finnish'  # Column for the A group frequency
B_frequency_column = 'Frequency Admixed American'     # 'Frequency African/African American'
A_allele_count = 'Allele Count European (non-Finnish)'
B_allele_count = 'Allele Count Admixed American' #'Allele Count African/African American'
A_to_B_frequency_ratio_column = 'European non-Finnish Frequency/Admixed Frequency' #'European non-Finnish Frequency/African African American Frequency'
B_to_A_frequency_ratio_column =  'Admixed Frequency/European non-Finnish Frequency' # 'African African American Frequency/European non-Finnish Frequency'
