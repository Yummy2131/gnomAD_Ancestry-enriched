        ...
        if filename.endswith('.xls') or filename.endswith('.xlsx'):
            ...
            try:
                ...
                Group_B_Enriched = df[(df[group_column] == Group_B) &
                                      (df[B_frequency_column] > 0.00005) &
                                      (df[B_frequency_column] < 0.01) &
                                      (df[B_allele_count] > 2) & ((df[B_to_A_frequency_ratio_column] >= 10) | (df[B_to_A_frequency_ratio_column].isna()))]
...
output_file = f'C:\\Users\\boxx_\\Desktop\\gnomAD\\{file_path}\\NFE_AFR_{file_path}.xlsx'
